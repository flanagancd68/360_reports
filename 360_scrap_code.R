...
# # write.csv(HIM.CAC001d, file = paste0("./360allhub/HIM.CAC001d_", qtr,"_",hub,".csv"))
# ### GET CODER LIST
# 
# HIM.CAC001d<- read_csv("HIM.CAC001d.csv", 
#                        col_types = cols(`% Precision` = col_number(), `% Recall` = col_number(), Accepted = col_number(), 
...
# 
# ### using automatic totals in 360 report, create one row per coder file based on fac=Total
# ## add new variables
# ### ################################################
# 
...
#   replace(is.na(.),0)
# 
# write.csv(HIM.CAC003_cdr, file = paste0("./360allhub/HIM.CAC003_cdr", qtr,"_",hub,".csv"))
# ## FACILITY
# ## 
...
#   replace(is.na(.),0)
# 
# write.csv(HIM.CAC003_cdr, file = paste0("./360allhub/HIM.CAC003_fac", qtr,"_",hub,".csv"))
...
# 
# #write.csv(HIM.CAC007d_cdr, file = paste0("./360allhub/HIM.CAC007_cdr_", qtr,"_",hub,".csv"))
# ##############################
# ##### Facility   ############
# ##### ######################
# 
...
# ### Large file size - causes 360 to crash
...
# 
# write.csv(HIM.CAC008, file = paste0("./360allhub/HIM.CAC008_", qtr,"_",hub,".csv"))
# ###########################################################################################################################
...
#   replace(is.na(.),0) %>% 
#   filter(`Disch Date` > 0)  %>%  #removes 360 subtotal rows
#   select(-Coder, -`No Second Group`, -`Total Visits`, -`MRN`, -Name, -`Admit Date`) %>% #removes 360 subtotal columns and other unneeded columns - edit as desired
...
# #write.csv(HIM.Prod016, file = paste0("./360allhub/HIM.Prod016_", qtr,"_",hub,".csv"))
# #
# #
# # join 1:1 coder tables together
# HIM.CAC_cdr <- left_join(HIM.CAC_cdr, HIM.Prod016)
# rm(HIM.Prod016)
# write.csv(HIM.CAC_cdr, file = paste0("./360allhub/HIM.CAC_cdr_", qtr,"_",hub,".csv"))
# 
# 
# ###########################################################################################################################
# ###########################################################################################################################
...
# AllinternalFac <- read_csv("4/AllinternalFac.csv")
# HIM_IP004 <- read_csv("4/HIM.IP004.csv", col_types = cols(Acct = col_character()))
# HIM_IP004 <- left_join(HIM_IP004, AllinternalFac)
# 
# AllinternalDRG_Dx3_APR <- read_csv("4/AllinternalDRG_Dx3_APR.csv", 
#                                 col_types = cols(`APR` = col_number(), 
#                                                  acct = col_character(), disc_disp = col_number(), 
#                                                  disc_dt = col_date(format = "%m/%d/%Y"), 
#                                                  drg = col_number()))
# AllinternalDRG_noDx3_APR <- read_csv("4/AllinternalDRG_noDx3_APR.csv", 
#                                   col_types = cols(`APR` = col_number(), 
#                                                    disc_disp = col_number(), disc_dt = col_date(format = "%m/%d/%Y"), 
#                                                    drg = col_number(), drg_ty = col_character()))
# AllinternalDRG <- full_join(AllinternalDRG_Dx3_APR, AllinternalDRG_noDx3_APR)
# HIM_IP004_n <- inner_join(HIM_IP004, AllinternalDRG)
# 
# 
# #summarize by DRG
